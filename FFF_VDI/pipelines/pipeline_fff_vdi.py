...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def ddim_inversion(self, latent, image_latents, masks, flows, cond, added_time_ids):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            for i, t in enumerate(tqdm(timesteps)):
                ...
                eps = self.unet(latent_model_input, masks, flows, t, encoder_hidden_states=cond, added_time_ids=added_time_ids).sample
    ...
    def __call__(
        ...
        masks: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        height: int = 576,
        width: int = 1024,
        ...
        for input_idx in range(len(masks)):
            ...
            total_masks.append(input_mask)
        total_masks = torch.cat(total_masks, dim=1).to(device, dtype=image_embeddings.dtype)
        total_masks = total_masks[:, :, 0:1, :, :]

        masked_image_values = total_images * (1 - total_masks)
        first_pixel_value = masked_image_values[:, 0, :, :, :]
        ...
        for input_idx in range(0, len(images)):
            ...
            condition_latent = self._encode_vae_image(
                masked_image,
                device=device,
